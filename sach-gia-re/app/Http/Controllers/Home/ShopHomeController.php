...
class ShopHomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('home.shop.list',[
            ...
            'shops' => $this->shopService->getShopPageHome()
        ]);
    }

    public function show(Request $request, $id='',$slug='')
    {
        $shop = $this->shopService->getID($id);

        if(isset($_GET['sort_by'])){
            $sort_by = $_GET['sort_by'];
            $coupons = $this->shopService->getCouponSortBy($shop,$sort_by);
        }else{
            $coupons = $this->shopService->getCoupon($shop);
        }
